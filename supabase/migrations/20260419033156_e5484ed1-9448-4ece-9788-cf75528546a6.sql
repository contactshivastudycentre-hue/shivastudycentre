...
-- Backfill: any existing prize_pool becomes Money type
UPDATE public.tests
SET prize_type = 'Money',
    prize_value = '₹' || prize_pool::text
WHERE prize_pool IS NOT NULL AND prize_type IS NULL;

-- Rename existing Sunday Special test to new branding
UPDATE public.tests
SET title = REGEXP_REPLACE(title, 'SUNDAY SPECIAL', 'SPECIAL', 'gi'),
    updated_at = now()
WHERE title ILIKE '%SUNDAY SPECIAL%';

-- 2) Update sync_test_banner to include prize_type, prize_value, prize_description
...
-- Re-fire trigger on existing rows so banner row is rebuilt with new title + prize meta
UPDATE public.tests SET updated_at = now()
WHERE test_type IN ('sunday_special','weekly','surprise_quiz') AND is_published = true;